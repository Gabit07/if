...
int CChain::FastChain(int x1, int y1, int x2, int y2)
{
	 ...
	 // initialize 
	 if(m_skipx<2) m_skipx=2;              // Skip하는 픽셀이 2보다 작을수 없다.
	 if(m_skipy<2) m_skipy=2;              //
	 if(m_minboundary<5) m_minboundary=5;  // 구한 chain의 길이가 5보다 작을수 없다.
...
double CChain::FindCompactness(int N)
{
	 ...
	 dArea   = Chain_LogicalArea(N);//Chain_Area(N);
	 int testval = (int)(dArea/dLength);
...
//ty=ax+b;
	...
	else
	{
		...
		pos[0][1]=BPixel[1];
		pos[1][0]=BPixel[0];
		pos[1][1]=BPixel[(number-1)*2+1];
		pos[2][0]=BPixel[(number-1)*2];
		pos[2][1]=BPixel[(number-1)*2+1];
		pos[3][0]=BPixel[(number-1)*2];
		pos[3][1]=BPixel[1];
	}
